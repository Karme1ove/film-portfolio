...
// ZMIANA: Dodano 'Captions' do importu ikon
...
interface StalkerPlayerProps {
    ...
    poster?: string; // Opcjonalnie okładka
}
...
export default function StalkerPlayer({ src, poster }: StalkerPlayerProps) {
    ...
    const [currentSubtitle, setCurrentSubtitle] = useState<number>(-1); // -1 = Wyłączone
    ...
    useEffect(() => {
        ...
        if (Hls.isSupported()) {
            const hls = new Hls({
                ...
                // Wymuszamy wyłączenie napisów na starcie, jeśli hls.js próbuje je włączyć
                enableWebVTT: true,
                subtitlePreference: {
                    lang: '' // Puste, żeby nie dobierał auto
                }
            });
            ...
            hls.on(Hls.Events.MANIFEST_PARSED, (event, data) => {
                ...
                // 2. Wymuś wyłączenie napisów na start (dla pewności)
                hls.subtitleTrack = -1;
                setCurrentSubtitle(-1);
            });

            // Wykrywanie ścieżek napisów
            ...
            // Safari Fallback
            video.src = src;
            // W Safari napisy obsługuje się przez video.textTracks,
            // ale tutaj zostawiamy natywne zachowanie lub trzeba by dorobić obsługę natywną.
        }
    ...
    // --- SCRUBBING LOGIC ---
    // (Skrócona dla czytelności - logika identyczna jak w poprzednim pliku)
    ...
    const startTimelineScrub = (e: any) => {
        e.stopPropagation(); setIsScrubbingTime(true);
        handleTimelineScrub(getClientX(e));
    };
    ...
    const startVolumeScrub = (e: any) => {
        e.stopPropagation(); setIsScrubbingVolume(true);
        handleVolumeScrub(getClientX(e));
    };
    ...
    // Wykrywanie fullscreena dla ikon
    useEffect(() => {
        ...
        <div className="w-full flex flex-col gap-4">
            ...
            >
                ...
                <video
                    ...
                    onPlay={() => {setIsPlaying(true); showControlsTemporary();}}
                ...
                <div className={`absolute bottom-0 left-0 w-full bg-gradient-to-t from-black via-black/90 to-transparent pt-12 pb-4 px-4 md:px-6 flex flex-col gap-3 z-30 transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0'}`} onClick={(e)=>e.stopPropagation()}>
                    ...
                    <div ref={timelineRef} className="w-full h-6 flex items-center cursor-pointer group/timeline relative z-50 touch-none" onMouseDown={startTimelineScrub} onTouchStart={startTimelineScrub}>
                        <div className="w-full h-1 bg-zinc-800 relative pointer-events-none group-hover/timeline:h-1.5 transition-all">
                            <div className="h-full bg-red-700 relative" style={{ width: `${progressPercent}%` }}>
                                <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-red-500 shadow-[0_0_10px_rgba(220,38,38,1)] rounded-full scale-0 group-hover/timeline:scale-100 transition-transform" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between font-mono text-[10px] md:text-xs text-zinc-400 tracking-widest -mt-2">
                        <div className="flex items-center gap-4">
                            <button onClick={togglePlay} className="hover:text-white transition-colors p-2 -ml-2">{isPlaying ? <Pause size={16}/> : <Play size={16}/>}</button>
                            <span className="tabular-nums opacity-80">{formatTime(currentTime)} <span className="text-zinc-600">/</span> {formatTime(duration)}</span>
                        </div>

                        <div className="flex items-center gap-4">
                            {/* VOLUME */}
                            <div className="hidden sm:flex items-center gap-3 group/volume">
                                <button onClick={toggleMute} className="hover:text-white p-1">{isMuted || volume===0 ? <VolumeX size={16}/> : <Volume2 size={16}/>}</button>
                                <div ref={volumeRef} className="w-16 md:w-24 h-6 flex items-center cursor-pointer touch-none" onMouseDown={startVolumeScrub} onTouchStart={startVolumeScrub}>
                                    <div className="w-full h-1 bg-zinc-800 pointer-events-none"><div className="h-full bg-zinc-400 group-hover/volume:bg-red-700 transition-colors" style={{width:`${volumePercent}%`}}/></div>
                                </div>
                            </div>
                            ...
                            {subtitles.length > 0 && (
                                <div className="relative">
                                    <button
                                        ...
                                        className={`hover:text-red-500 transition-colors p-1 flex items-center gap-2 ${showSubtitleMenu ? 'text-red-500' : ''}`}
                                        title="Napisy"
                                    >
                                        <Captions size={16} />
                                        <span className="hidden sm:inline opacity-50 text-[9px]">
                                            {currentSubtitle === -1 ? 'OFF' : subtitles[currentSubtitle]?.lang.toUpperCase()}
                                        </span>
                                    </button>

                                    {showSubtitleMenu && (
                                        <div className="absolute bottom-full right-0 mb-4 bg-black border border-white/10 shadow-xl p-1 min-w-[120px] flex flex-col z-50">
                                            <button
                                                onClick={() => changeSubtitle(-1)}
                                                className={`px-4 py-2 text-left hover:bg-zinc-900 hover:text-red-500 transition-colors text-[10px] tracking-widest ${currentSubtitle === -1 ? 'text-red-500' : 'text-zinc-400'}`}
                                            >
                                                [ WYŁĄCZ ]
                                            </button>
                                            {subtitles.map((track) => (
                                                <button
                                                    ...
                                                    className={`px-4 py-2 text-left hover:bg-zinc-900 hover:text-red-500 transition-colors text-[10px] tracking-widest uppercase ${currentSubtitle === track.id ? 'text-red-500' : 'text-zinc-400'}`}
                                                >
                            ...
                            {qualities.length > 0 && (
                                <div className="relative">
                                    <button
                                        ...
                                        className={`hover:text-red-500 transition-colors p-1 flex items-center gap-2 ${showQualityMenu ? 'text-red-500' : ''}`}
                                        title="Jakość"
                                    >
                                        <Settings size={16} />
                                        <span className="hidden sm:inline opacity-50 text-[9px]">
                                    ...
                                    {showQualityMenu && (
                                        <div className="absolute bottom-full right-0 mb-4 bg-black border border-white/10 shadow-xl p-1 min-w-[120px] flex flex-col z-50">
                                            <button onClick={() => changeQuality(-1)} className={`px-4 py-2 text-left hover:bg-zinc-900 hover:text-red-500 transition-colors text-[10px] tracking-widest ${currentQuality === -1 ? 'text-red-500' : 'text-zinc-400'}`}>[ AUTO ]</button>
                                            {qualities.map((q) => (
                                                <button key={q.index} onClick={() => changeQuality(q.index)} className={`px-4 py-2 text-left hover:bg-zinc-900 hover:text-red-500 transition-colors text-[10px] tracking-widest ${currentQuality === q.index ? 'text-red-500' : 'text-zinc-400'}`}>[ {q.height}P ]</button>
                                            ))}
                                        </div>
                                    )}
                                </div>
